...
class RecipeDetails extends StatelessWidget {
  final Recipe recipe; final bool isFavorite; final VoidCallback onFavorite;
  const RecipeDetails({super.key,required this.recipe,required this.isFavorite,required this.onFavorite});
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('রেসিপি'),actions:[IconButton(onPressed:onFavorite,icon:Icon(isFavorite?Icons.favorite:Icons.favorite_border,color:isFavorite?Colors.red:null))]),body:ListView(children:[AspectRatio(aspectRatio:16/10,child:Image.network(recipe.image,fit:BoxFit.cover,errorBuilder:(_,__,___)=>Container(color:Theme.of(context).colorScheme.surfaceContainerHighest,child:const Icon(Icons.restaurant,size:70)))),Padding(padding:const EdgeInsets.fromLTRB(18,18,18,28),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(recipe.name,style:Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight:FontWeight.w900)),const SizedBox(height:7),Text('${recipe.country} • ${recipe.category}'),const SizedBox(height:16),Row(children:[_stat(context,Icons.timer_outlined,recipe.time),const SizedBox(width:10),_stat(context,Icons.people_outline,'${recipe.servings} জন'),const SizedBox(width:10),_stat(context,Icons.local_fire_department_outlined,'সহজ')]),const SizedBox(height:20),Text(recipe.description,style:const TextStyle(fontSize:16,height:1.5)),const SizedBox(height:24),const Text('উপকরণ',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900)),const SizedBox(height:10),...recipe.ingredients.map((x)=>ListTile(contentPadding:EdgeInsets.zero,leading:const Icon(Icons.check_circle_outline),title:Text(x))),const SizedBox(height:14),Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[const Text('রান্নার নিয়ম',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900)),FilledButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>CookingMode(recipe:recipe))),icon:const Icon(Icons.play_arrow),label:const Text('Cooking Mode'))]),const SizedBox(height:8),...recipe.steps.asMap().entries.map((e)=>ListTile(contentPadding:EdgeInsets.zero,leading:CircleAvatar(child:Text('${e.key+1}')),title:Text(e.value,style:const TextStyle(height:1.4))))]))]);
  Widget _stat(BuildContext c,IconData icon,String text)=>Expanded(child:Container(padding:const EdgeInsets.all(10),decoration:BoxDecoration(color:Theme.of(c).colorScheme.surfaceContainerHighest,borderRadius:BorderRadius.circular(14)),child:Column(children:[Icon(icon,size:22),const SizedBox(height:3),Text(text,textAlign:TextAlign.center,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w600))])));
}

class CookingMode extends StatefulWidget { final Recipe recipe; const CookingMode({super.key,required this.recipe}); @override State<CookingMode> createState()=>_CookingState(); }
class _CookingState extends State<CookingMode>{int step=0; @override Widget build(BuildContext context){final r=widget.recipe;return Scaffold(appBar:AppBar(title:const Text('Cooking Mode')),body:Padding(padding:const EdgeInsets.all(24),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[Text('STEP ${step+1} / ${r.steps.length}',style:TextStyle(color:Theme.of(context).colorScheme.primary,fontWeight:FontWeight.w800)),const SizedBox(height:24),LinearProgressIndicator(value:(step+1)/r.steps.length,minHeight:8,borderRadius:BorderRadius.circular(8)),const Spacer(),const Icon(Icons.soup_kitchen_outlined,size:80),const SizedBox(height:30),Text(r.steps[step],textAlign:TextAlign.center,style:const TextStyle(fontSize:26,fontWeight:FontWeight.w800,height:1.35)),const Spacer(),Row(children:[if(step>0)Expanded(child:OutlinedButton(onPressed:()=>setState(()=>step--),child:const Text('← আগের ধাপ'))),if(step>0)const SizedBox(width:10),Expanded(child:FilledButton(onPressed:step<r.steps.length-1?()=>setState(()=>step++):()=>Navigator.pop(context),child:Text(step<r.steps.length-1?'পরের ধাপ →':'শেষ হয়েছে ✓')))]),const SizedBox(height:15)]));}}
